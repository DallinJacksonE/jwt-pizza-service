...
class DB {
  ...
  async listUsers(page = 0, limit = 10, filter = '*') {
    const connection = await this.getConnection();
    ...
    try {
      let users = await this.query(connection, `SELECT id, name, email FROM user WHERE name LIKE ? OR email LIKE ? LIMIT ${limit + 1} OFFSET ${offset}`, [filter, filter]);
